...
The whitelist algorithm has a different approach than the previous O2Balancer \cite{O2Balancer_github}. The previous version overrides a FairMQ class to allow for removing a channel from the channel list contained in MQProgOptions\cite{mq_prog}. The new version solves this by maintaining a list of the online nodes so it is known which channels not to use. This allows for significantly faster reinitializing of the node, since the channel doesn’t have to be broken down, instead the channels stay unused until the node comes back online in which case the node will be added to the list of online nodes. 
