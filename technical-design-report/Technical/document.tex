...
\begin{table}[H]
	...
	\begin{center}
		\begin{tabular}{ | c | c | }
			...
			\includegraphics[width=0.45\textwidth]{network-primary-interface} & \includegraphics[width=0.45\textwidth]{network-auxiliary-interface} \\ \hline
		\end{tabular}
...
\begin{center}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{"The effect on loss due to the unavailability of the auxiliary network interface"}
	\end{figure}
\end{center}
...
\begin{center}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{"Effects of increased event rates on the loss for Round Robin with 2 FLP's"}
	\end{figure}
\end{center}
...
\begin{center}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{"Effects of iteration time during round robin ICN experiment for 1 FLP"}
	\end{figure}
\end{center}

\begin{center}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{"Effects of iteration time during round robin ICN experiment for 2 FLP"}
	\end{figure}
\end{center}

\subsection{Metricbeat}
During the round robin ICN experiments metricbeat was used on the raspberry pi nodes to record metric data, unfortunately the results from this data proved unusable due to unreliability as well as the metricbeat process requiring a lot of system resources. The metricbeat process occasionally caused 65\% CPU utilization, additionally many graphs tended to be behind with actual system events it hard to correlate them. Many of the nodes stopped correctly reporting metrics after a period of time which required the restarting of the metricbeat process to be resolved.

\begin{center}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{metric-load}
...
\section{Conclusion}
200Hz event rate should be manageable with only a small reduction in throughput by round robin algorithm, additional experiments with more nodes required

Metricbeat could be useful but requires a lot of work to correctly configure along with Logstash, Elasticsearch \& Kibana. It might be unfeasible to run this service on ARM based nodes and additional work is required to determine if it feasible.

Future experiments should take multiple iteration times into account as this parameter can effect results. Perhaps previous experiments should be repeated with different iteration times.
